...
async function main() {
  ...
  console.log(`Checking Account Balance for address ${voterAddress}`)

  const votingPower = await deployedContract.balanceOf(voterAddress);

  console.log(
        `The Account Balance for account with address ${voterAddress} is ${ethers.utils.formatEther(votingPower)} tokens`)
}
